...
def read_csv(filename):
    ...
    if len(open(filename).readlines()) > 1000:
...
if __name__=="__main__":
    ...
    #import info
    logger.info("start update company default setting")
    load_file(conn, "res.company", "company.default.csv", logger)

    models = xmlrpclib.ServerProxy('{}/xmlrpc/2/object'.format(url))   
    update_values_cash = models.execute_kw(conn.db, conn.uid, conn.password, 'account.account', 'search', 
         [[('company_id','=',1),('name','=','Cash')]])
    if len(update_values_cash) == 0:
         raise ValueError('no Cash account data in company 1 , or no main company')
    
    update_values_bank = models.execute_kw(conn.db, conn.uid, conn.password, 'account.account', 'search', 
         [[('company_id','=',1),('name','=','Bank')]])
    if len(update_values_bank) == 0:
         raise ValueError('no Bank account data in company 1')

    target_ids = models.execute_kw(conn.db, conn.uid, conn.password, 'res.company','search',
         [[('id','=',1)]])
    
    models.execute_kw(conn.db, conn.uid, conn.password, 'res.company', 'write', 
         [target_ids,
         {'cash_account_id':update_values_cash[0],
		 'check_account_id':update_values_bank[0]}])
