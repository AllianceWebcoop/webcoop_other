...
def load_file(conn, table, filename, logger):
    ...
    #update interest_rate,penalty_rate,etc from loan configuration
    for tid in res['ids']:
        #get config
	...
	#get self data for update loan header 
        ...
        models.execute_kw(conn.db, conn.uid, conn.password, 'wc.loan', 'write', 
            ...
            {'date_maturity':loan_header[0]['note'],
             'note':'this loan is migrated data as paid loan'}])

        logger.info("start loan detail update")
        #update loan detail
	loan_detail_data = models.execute_kw(conn.db, conn.uid, conn.password, 'wc.loan.detail', 'search_read',
            [[['loan_id','=',tid]]],
            {'fields':['principal_due','interest_due','principal_paid','interest_paid','id']})
...
if __name__=="__main__":
    ...
    load_file(conn, "wc.loan", "loan_paid.csv", logger)
    #******************************************************************************	
